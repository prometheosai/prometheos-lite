...
On Windows, use `py -3` instead of `python3`. The `platform` suite must run independently on owned Linux, macOS, and Windows machines. The smoke suite is Unix-only. Frontend changes additionally require:
...
The verifier requires Linux `core`, `platform`, and `smoke` evidence plus macOS and Windows `platform` evidence. Every record must match the exact commit, report a clean tree, contain only passing checks, and retain its evidence digest. Frontend evidence is additionally required when frontend paths change.
