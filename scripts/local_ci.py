...
def unix_smoke() -> list[dict[str, object]]:
    if platform.system() == "Windows":
        raise RuntimeError("smoke suite requires Linux or macOS; use an owned Unix host")
    commands = [
    ...
    binary = install_root / "bin" / "prometheos"
...
def verify_evidence(paths: list[str], commit: str) -> None:
    ...
    required = {(os_name, "platform") for os_name in ("linux", "darwin", "windows")}
    required |= {("linux", "core"), ("linux", "smoke")}
...
def main() -> int:
    ...
    verify_parser = sub.add_parser("verify", help="verify collected cross-platform evidence")
    verify_parser.add_argument("--commit", required=True)
    verify_parser.add_argument("evidence", nargs="+")
    args = parser.parse_args()

    if args.command == "verify":
        verify_evidence(args.evidence, args.commit)
        return 0
