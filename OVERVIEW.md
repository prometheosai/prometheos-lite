...
### `src/flow/runtime.rs` - Runtime Context
...
* `RuntimeContext::new`
* `RuntimeContext::with_model_router`
* `RuntimeContext::with_tool_runtime`
* `RuntimeContext::with_memory_service`
* `RuntimeContext::full`

Interaction paths:

* Used by CLI to package services for injection into nodes.
* Services injected into `DefaultNodeFactory` for node creation.

Patterns:

* Centralized service registry.
* Builder pattern for optional services.
...
| Flow runner         | `src/cli/runner.rs`                                         | `@utility`  | Loads flow JSON files and executes with service injection.       |
...
| Runtime context     | `src/flow/runtime.rs`                                       | `@core`     | Service registry for ModelRouter, ToolRuntime, MemoryService.    |
...
1. **Persist run registry and flow events** to SQLite so `Maestro` survives process restarts.
2. **Replace brute-force semantic search with indexed vector retrieval** once memory volume grows; SQLite extension or a pluggable vector backend would fit the current local-first design.
3. **Retire or isolate legacy `agents` / `core`** after parity is proven, keeping `AgentNode` only as a migration shim rather than permanent architecture furniture.
4. **Unify debug and production execution** by adding lifecycle hooks to `Flow::run` instead of duplicating execution inside `DebugSession`.

## Historical Note: Legacy Architecture

The original v1.0 architecture used a sequential agent-based system with the following components:

- **SequentialOrchestrator**: Linear pipeline executing Planner → Coder → Reviewer
- **Agent trait**: Async trait defining agent interface with `name()` and `run()` methods
- **PlannerAgent**: Task planning with structured markdown output
- **CoderAgent**: Code generation with file block output
- **ReviewerAgent**: Code review and refinement

This architecture has been deprecated in favor of the flow-centric system described above. The legacy code remains behind the `legacy` feature flag for backward compatibility and migration purposes via `AgentNode` adapter and `migration` utilities.
