...
impl ArtifactGenerator {
    ...
    pub fn generate(&self, kind: ArtifactKind, content: String, metadata: ArtifactMetadata) -> Result<HarnessArtifact> {
        let size_bytes = content.len();
        let (compressed_content, compressed_size, compression) = if self.compression_enabled && size_bytes > self.compression_threshold_bytes {
            let compressed = self.compress(&content, self.default_compression)?;
            let compressed_size = compressed.len();
            (Some(compressed), Some(compressed_size), self.default_compression)
        } else {
        ...
        Ok(HarnessArtifact {
            ...
            kind,
            path: None,
            content: if compressed_content.is_some() { None } else { Some(content) },
            compressed_content,
            compression,
            metadata,
            created_at: Utc::now(),
    ...
    pub fn generate_patch_artifact(&self, patch_content: &str, metadata: ArtifactMetadata) -> Result<HarnessArtifact> {
        self.generate(ArtifactKind::Patch, patch_content.to_string(), metadata)
    }

    pub fn generate_report_artifact(&self, report_content: &str, metadata: ArtifactMetadata) -> Result<HarnessArtifact> {
        self.generate(ArtifactKind::Report, report_content.to_string(), metadata)
    }

    pub fn generate_trace_artifact(&self, trace_content: &str, metadata: ArtifactMetadata) -> Result<HarnessArtifact> {
        self.generate(ArtifactKind::Trace, trace_content.to_string(), metadata)
    }

    pub fn generate_evidence_artifact(&self, evidence_content: &str, metadata: ArtifactMetadata) -> Result<HarnessArtifact> {
        self.generate(ArtifactKind::Evidence, evidence_content.to_string(), metadata)
    }

    pub fn generate_log_artifact(&self, log_content: &str, metadata: ArtifactMetadata) -> Result<HarnessArtifact> {
        self.generate(ArtifactKind::Log, log_content.to_string(), metadata)
    }

    pub fn generate_metrics_artifact(&self, metrics_content: &str, metadata: ArtifactMetadata) -> Result<HarnessArtifact> {
        self.generate(ArtifactKind::Metrics, metrics_content.to_string(), metadata)
    }

    pub fn generate_all_artifacts(&self, result: &HarnessExecutionResult, work_context_id: String) -> Result<Vec<HarnessArtifact>> {
    ...
    fn compress(&self, content: &str, compression: CompressionType) -> Result<Vec<u8>> {
        match compression {
            ...
            CompressionType::Zstd => {
                Ok(zstd::encode_all(content.as_bytes(), 3)?)
            }
    ...
    pub fn decompress(&self, artifact: &HarnessArtifact) -> Result<String> {
        match artifact.compression {
            ...
            CompressionType::Gzip => {
                use std::io::Read;
                let compressed = artifact.compressed_content.as_ref().ok_or_else(|| anyhow::anyhow!("No compressed content"))?;
...
pub fn format_artifact_summary(artifact: &HarnessArtifact) -> String {
    let compression_ratio = artifact.compressed_size_bytes.map(|c| {
        let ratio = (artifact.size_bytes as f64 - c as f64) / artifact.size_bytes as f64 * 100.0;
