...
pub async fn fingerprint_environment(root: &Path) -> Result<EnvironmentProfile> {
    ...
    if profile.languages.is_empty() {
        ...
        profile.warnings.push("No recognized project type detected".into());
    }
...
fn detect_rust(root: &Path, profile: &mut EnvironmentProfile) -> Result<()> {
    if root.join("Cargo.toml").exists() {
        ...
        profile.lint_commands.push("cargo clippy -- -D warnings".into());
        profile.test_commands.push("cargo test".into());
        profile.test_commands.push("cargo test --all-features".into());
    ...
    if root.join("package.json").exists() {
        ...
        if let Some(scripts) = scripts {
            ...
            if scripts.contains_key("type-check") || scripts.contains_key("tsc") {
                profile.type_check_commands.push(format!("{} run type-check", pm));
            }
        }
        ...
        if has_typescript {
            ...
            if profile.format_commands.is_empty() {
                profile.format_commands.push("npx prettier --check .".into());
            }
        }
        ...
        if root.join("next.config.js").exists() || root.join("next.config.ts").exists() {
            ...
            profile.warnings.push("Next.js detected - ensure NODE_ENV is set correctly".into());
        }
    ...
    if has_requirements || has_pyproject || has_setup_py || has_pipfile || has_poetry {
        ...
        match pm {
            "poetry" => {
                ...
                profile.format_commands.push("poetry run ruff format --check .".into());
            ...
            _ => {
                ...
                profile.test_commands.push("python -m unittest discover".into());
        ...
        if root.join("conda.yml").exists() || root.join("environment.yml").exists() {
            ...
            profile.warnings.push("Conda environment detected - ensure environment is activated".into());
        }
    }
...
fn detect_java(root: &Path, profile: &mut EnvironmentProfile) -> Result<()> {
    ...
    if has_pom || has_gradle || has_gradle_kts {
        ...
        if has_pom {
            ...
            profile.build_commands.push("mvn package -DskipTests".into());
        ...
        if has_gradle || has_gradle_kts {
            ...
            profile.build_commands.push("./gradlew build -x test".into());
            profile.test_commands.push("./gradlew test".into());
            profile.lint_commands.push("./gradlew checkstyleMain".into());
            profile.format_commands.push("./gradlew spotlessCheck".into());
...
fn detect_go(root: &Path, profile: &mut EnvironmentProfile) -> Result<()> {
    ...
    if has_go_mod {
        ...
        if root.join("vendor").is_dir() {
            profile.detected_files.push("vendor/ (vendored deps)".into());
        }
...
fn detect_php(root: &Path, profile: &mut EnvironmentProfile) -> Result<()> {
    ...
    if has_composer {
        ...
        profile.lint_commands.push("./vendor/bin/phpstan analyse".into());
        profile.format_commands.push("./vendor/bin/php-cs-fixer fix --dry-run".into());
    }
...
fn detect_docker(root: &Path, profile: &mut EnvironmentProfile) -> Result<()> {
    ...
    if has_dockerfile || has_docker_compose || has_k8s {
        ...
        if has_docker_compose {
                        ...
                        for (name, _) in services {
                            if let Some(name_str) = name.as_str() {
                                ...
                                profile.services.push(ServiceDependency {
                                    ...
                                    health_check: Some(format!("docker compose ps {} | grep healthy", name_str)),
                                    port: None,
                                });
                            }
                        }
            ...
            profile.test_commands.push("docker compose run --rm app test".into());
        ...
        if has_k8s {
            ...
            profile.warnings.push("Kubernetes configuration detected - requires cluster access for testing".into());
        }
...
fn detect_ci(root: &Path, profile: &mut EnvironmentProfile) -> Result<()> {
    ...
    if github_workflows.is_dir() {
                ...
                if let Some(ext) = entry.path().extension() {
                    if ext == "yml" || ext == "yaml" {
                        if let Ok(content) = fs::read_to_string(entry.path()) {
                            if content.contains("test") || content.contains("pytest") || content.contains("cargo test") {
                                ci_config.test_command = Some("github actions test workflow".into());
                            }
                            if content.contains("lint") || content.contains("clippy") || content.contains("eslint") {
        ...
        profile.warnings.push("Travis CI detected - consider migrating to GitHub Actions".into());
    }
...
fn detect_env_files(root: &Path, profile: &mut EnvironmentProfile) -> Result<()> {
    if root.join(".env").exists() {
        ...
        profile.warnings.push("WARNING: .env file detected in repository - ensure secrets are not committed".into());
    }
    ...
    for pattern in &sensitive_patterns {
        if root.join(pattern).exists() {
            profile.warnings.push(format!("CRITICAL: Potential sensitive file detected: {}", pattern));
        }
    }
...
pub fn can_run_in_container(profile: &EnvironmentProfile) -> bool {
    profile.container_config.as_ref()
