...
impl AcceptanceCompiler {
    ...
    pub fn compile(&self, reqs: &[String], env: &EnvironmentProfile) -> CompiledAcceptanceCriteria {
        ...
        for (i, req) in reqs.iter().enumerate() {
            ...
            } else if matches!(criterion.verification_method, VerificationMethod::Review | VerificationMethod::Manual) {
        ...
        let total_priority_score = criteria.iter()
    ...
    fn analyze_criterion(&self, req: &str, index: usize, env: &EnvironmentProfile) -> (AcceptanceCriterion, Vec<String>, Vec<String>, Vec<String>, Vec<String>) {
        ...
        let confidence = self.calculate_detection_confidence(req, &detected_tests, &detected_checks);
        ...
        let verification_method = if !detected_tests.is_empty() {
            ...
            let path = self.extract_file_path(req).unwrap_or_else(|| PathBuf::from("unknown"));
        ...
        (criterion, detected_tests, detected_checks, detected_lints, detected_formats)
    }
    ...
    fn infer_priority(&self, req: &str) -> CriterionPriority {
        ...
        if req_lower.contains("critical") || req_lower.contains("security") || 
           req_lower.contains("crash") || req_lower.contains("data loss") {
            CriterionPriority::Critical
        } else if req_lower.contains("must") || req_lower.contains("required") ||
                  req_lower.contains("important") || req_lower.contains("ensure") {
    ...
    fn extract_file_path(&self, req: &str) -> Option<PathBuf> {
        let file_pattern = Regex::new(r"(?i)(?:create|add|file)\s+(?:`?)([\w./-]+\.\w+)(?:`?)").ok()?;
        file_pattern.captures(req)
...
pub fn get_verification_summary(criteria: &[AcceptanceCriterion]) -> String {
    ...
    let passed = criteria.iter().filter(|c| matches!(c.status, CriterionStatus::Passed)).count();
    let failed = criteria.iter().filter(|c| matches!(c.status, CriterionStatus::Failed)).count();
    let pending = criteria.iter().filter(|c| matches!(c.status, CriterionStatus::Pending)).count();
    let critical = criteria.iter().filter(|c| c.priority == CriterionPriority::Critical).count();
    
    let auto_verifiable = criteria.iter()
        .filter(|c| !matches!(c.verification_method, VerificationMethod::Review | VerificationMethod::Manual))
        .count();
