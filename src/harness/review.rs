...
impl ReviewEngine {
    pub fn new() -> Self {
        Self {
            security_patterns: vec![
                ...
                (Regex::new(r"(?i)exec\s*\(").unwrap(), ReviewSeverity::High, "SEC008"),
                (Regex::new(r"(?i)innerHTML\s*=").unwrap(), ReviewSeverity::High, "SEC009"),
                (Regex::new(r"(?i)document\.write").unwrap(), ReviewSeverity::High, "SEC010"),
                (Regex::new(r"(?i)\.env\[").unwrap(), ReviewSeverity::Medium, "SEC011"),
                (Regex::new(r"(?i)TODO.*security|FIXME.*security|XXX.*security").unwrap(), ReviewSeverity::Medium, "SEC012"),
            ],
            bug_patterns: vec![
                ...
                (Regex::new(r"(?i)fixme|xxx|hack").unwrap(), ReviewSeverity::Low, "BUG006"),
                (Regex::new(r"(?i)clone\(\)").unwrap(), ReviewSeverity::Low, "BUG007"),
                (Regex::new(r"(?i)as_ptr\(\)").unwrap(), ReviewSeverity::Medium, "BUG008"),
                (Regex::new(r"(?i)std::mem::transmute").unwrap(), ReviewSeverity::High, "BUG009"),
                (Regex::new(r"(?i)null\s*check|NPE|nullptr").unwrap(), ReviewSeverity::High, "BUG010"),
            ],
            performance_patterns: vec![
                (Regex::new(r"(?i)for\s+.*\.len\(\)").unwrap(), ReviewSeverity::Low, "PERF001"),
                (Regex::new(r"(?i)\.collect::<Vec<_>>\(\)").unwrap(), ReviewSeverity::Low, "PERF002"),
                (Regex::new(r"(?i)String::new\(\)").unwrap(), ReviewSeverity::Info, "PERF003"),
                (Regex::new(r"(?i)\.to_string\(\)").unwrap(), ReviewSeverity::Low, "PERF004"),
                (Regex::new(r"(?i)Box::new\(").unwrap(), ReviewSeverity::Low, "PERF005"),
                (Regex::new(r"(?i)Rc<|Arc<").unwrap(), ReviewSeverity::Low, "PERF006"),
                (Regex::new(r"(?i)Mutex<|RwLock<").unwrap(), ReviewSeverity::Medium, "PERF007"),
                (Regex::new(r"(?i)thread::spawn").unwrap(), ReviewSeverity::Medium, "PERF008"),
                (Regex::new(r"(?i)sleep\(").unwrap(), ReviewSeverity::Low, "PERF009"),
                (Regex::new(r"(?i)\.clone\(\).*\.clone\(\)").unwrap(), ReviewSeverity::Medium, "PERF010"),
            ],
            maintainability_patterns: vec![
                (Regex::new(r"(?i)fn\s+\w+\([^)]{80,}\)").unwrap(), ReviewSeverity::Medium, "MAINT001"),
                (Regex::new(r"(?i)if\s+.*\{\s*\n\s*if").unwrap(), ReviewSeverity::Medium, "MAINT002"),
                (Regex::new(r"(?i)match.*\{\s*\n.*match").unwrap(), ReviewSeverity::Medium, "MAINT003"),
                (Regex::new(r"(?i)/\*.*\*/").unwrap(), ReviewSeverity::Low, "MAINT004"),
                (Regex::new(r"(?i)(println!|eprintln!)").unwrap(), ReviewSeverity::Low, "MAINT005"),
                (Regex::new(r"(?i)(dbg!|debug!)").unwrap(), ReviewSeverity::Low, "MAINT006"),
                (Regex::new(r"(?i)magic\s+number|hardcoded").unwrap(), ReviewSeverity::Medium, "MAINT007"),
                (Regex::new(r"(?i)const\s+\w+\s*=\s*\d+").unwrap(), ReviewSeverity::Info, "MAINT008"),
            ],
            test_patterns: vec![
                (Regex::new(r"(?i)#\[test\]").unwrap(), ReviewSeverity::Info, "TEST001"),
                (Regex::new(r"(?i)#\[cfg\(test\)\]").unwrap(), ReviewSeverity::Info, "TEST002"),
                (Regex::new(r"(?i)mod\s+tests\s*\{").unwrap(), ReviewSeverity::Info, "TEST003"),
                (Regex::new(r"(?i)fn\s+test_").unwrap(), ReviewSeverity::Info, "TEST004"),
                (Regex::new(r"(?i)assert!|assert_eq!|assert_ne!").unwrap(), ReviewSeverity::Info, "TEST005"),
            ],
            style_patterns: vec![
                (Regex::new(r"(?i)TODO:|FIXME:|XXX:").unwrap(), ReviewSeverity::Low, "STYLE001"),
                (Regex::new(r"(?i)\t").unwrap(), ReviewSeverity::Low, "STYLE002"),
                (Regex::new(r"(?i)  +$").unwrap(), ReviewSeverity::Info, "STYLE003"),
                (Regex::new(r"(?i)if\s*\(\s*").unwrap(), ReviewSeverity::Info, "STYLE004"),
                (Regex::new(r"(?i)fn\s+\w+[A-Z]").unwrap(), ReviewSeverity::Low, "STYLE005"),
            ],
            doc_patterns: vec![
                (Regex::new(r"(?i)^\s*///").unwrap(), ReviewSeverity::Info, "DOC001"),
                (Regex::new(r"(?i)^\s*//!").unwrap(), ReviewSeverity::Info, "DOC002"),
                (Regex::new(r"(?i)^\s*#\[doc").unwrap(), ReviewSeverity::Info, "DOC003"),
                (Regex::new(r"(?i)pub\s+fn\s+\w+\s*\([^)]*\)\s*->\s*[^{]*\{\s*\n").unwrap(), ReviewSeverity::Low, "DOC004"),
            ],
        }
    }
    ...
    pub fn review_diff(&self, diff: &str, file_path: Option<&Path>) -> Vec<ReviewIssue> {
        ...
        if self.is_api_change(diff) {
            issues.push(ReviewIssue {
                ...
                suggestion: Some("Ensure backward compatibility or document breaking changes".to_string()),
        ...
        if self.is_dependency_change(diff) {
            issues.push(ReviewIssue {
                ...
                suggestion: Some("Review dependency changes for security and compatibility".to_string()),
    ...
    fn detect_performance_issues(&self, context: &ReviewContext, file_str: &str) -> Vec<ReviewIssue> {
        let mut issues = vec![];
        let content_lower = context.file_content.to_lowercase();
    ...
    fn detect_maintainability_issues(&self, context: &ReviewContext, file_str: &str) -> Vec<ReviewIssue> {
        let mut issues = vec![];
        let content_lower = context.file_content.to_lowercase();
    ...
    fn detect_test_issues(&self, context: &ReviewContext, file_str: &str) -> Vec<ReviewIssue> {
        let mut issues = vec![];
        let content_lower = context.file_content.to_lowercase();
        let has_test_module = content_lower.contains("#cfg(test)") || 
        ...
        if has_pub_fn && !has_test_module && !file_str.contains("test") {
            ...
            let pub_fn_count = Regex::new(r"(?i)pub\s+fn\s+\w+").unwrap()
    ...
    fn detect_documentation_issues(&self, context: &ReviewContext, file_str: &str) -> Vec<ReviewIssue> {
        let mut issues = vec![];
        let content_lower = context.file_content.to_lowercase();
    ...
    fn parse_rust_ast(&self, context: &ReviewContext) -> Option<AstNode> {
        ...
        for cap in fn_pattern.captures_iter(&context.file_content) {
            if let Some(name) = cap.get(1) {
                let line = context.file_content[..cap.get(0).unwrap().start()].lines().count() + 1;
    ...
    fn analyze_ast(&self, ast: &AstNode, context: &ReviewContext, file_str: &str) -> Vec<ReviewIssue> {
        let mut issues = vec![];
    ...
    fn check_function_complexity(&self, ast: &AstNode, _context: &ReviewContext, file_str: &str, issues: &mut Vec<ReviewIssue>) {
    ...
    fn check_cyclomatic_complexity(&self, _ast: &AstNode, _context: &ReviewContext, _file_str: &str, _issues: &mut Vec<ReviewIssue>) {
...
pub fn generate_review_report(files: &[(PathBuf, String)]) -> ReviewReport {
    ...
    let critical_count = all_issues.iter().filter(|i| i.severity == ReviewSeverity::Critical).count();
    let high_count = all_issues.iter().filter(|i| i.severity == ReviewSeverity::High).count();
...
pub fn get_issues_by_type(report: &ReviewReport, issue_type: ReviewIssueType) -> Vec<&ReviewIssue> {
    report.issues.iter().filter(|i| i.issue_type == issue_type).collect()
}

pub fn get_issues_by_severity(report: &ReviewReport, severity: ReviewSeverity) -> Vec<&ReviewIssue> {
