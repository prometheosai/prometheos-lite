use crate::harness::{
    ...
    validation::{CommandResult, ValidationResult, ValidationCategory},
...
pub fn classify_patch_failure(failure: &PatchFailure) -> FailureKind {
    ...
    if reason_lower.contains("timeout") || failure.line_number.is_none() && reason_lower.contains("took too long") {
...
pub fn classify_validation_failure(result: &ValidationResult) -> FailureKind {
    let has_compile_errors = result.category_results.get(&ValidationCategory::Lint)
        .map(|r| !r.passed)
        .unwrap_or(false);
    
    let has_test_failures = result.category_results.get(&ValidationCategory::Test)
        .map(|r| !r.passed)
        .unwrap_or(false);
...
pub fn classify_command_failure(cmd: &CommandResult) -> FailureKind {
    ...
    if stderr_lower.contains("out of memory") || stderr_lower.contains("resource temporarily unavailable") {
...
pub fn create_failure_details(kind: FailureKind, message: impl Into<String>, context: FailureContext) -> FailureDetails {
...
fn generate_suggestion(kind: &FailureKind, message: &str, context: &FailureContext) -> Option<String> {
    match kind {
        FailureKind::PatchApplyFailure => {
            if message.contains("matched 0 times") {
                ...
                Some("Check file permissions and ensure the target file exists and is writable.".into())
            }
        }
        FailureKind::CompileFailure => {
            if let Some(ref file) = context.file {
                Some(format!("Check syntax in {} and verify all imports/dependencies are available.", file.display()))
            } else {
                Some("Review the code for syntax errors or missing dependencies.".into())
            }
        }
        FailureKind::TestFailure => {
            Some("Review test output to identify the failing assertions and expected vs actual values.".into())
        }
        FailureKind::TimeoutFailure => {
            Some("Consider increasing timeout limits or breaking the operation into smaller chunks.".into())
        }
        FailureKind::PermissionFailure => {
            Some("Check file permissions and ensure the process has appropriate access rights.".into())
        }
...
impl FailureAggregator {
    ...
    pub fn has_fatal(&self) -> bool {
        self.failures.iter().any(|f| f.severity == FailureSeverity::Fatal)
    }
