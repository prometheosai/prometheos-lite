    ...
    for file in &files {
        ...
        *language_breakdown.entry(lang.clone()).or_insert(0) += 1;
        ...
        if let Ok(content) = fs::read_to_string(file) {
            if let Some((symbols, relationships)) = extract_symbols_and_relationships(file, &content, &lang) {
                for sym in &symbols {
...
fn extract_symbols_and_relationships(
    ...
    extract_from_node(file, content, &root, &ts_lang, &mut symbols, &mut relationships);
    ...
    match kind {
        ...
        "import_statement" | "use_declaration" => {
            ...
            for name in imported_names {
                relationships.push(SymbolEdge {
                    from: file.file_name().unwrap_or_default().to_string_lossy().to_string(),
                    to: name,
                    file: file.to_path_buf(),
                    line: line_start,
...
fn rank_files_by_relevance(
    ...
    for sym in symbols {
        symbol_index.entry(sym.name.to_lowercase()).or_default().push(sym);
    }
        ...
        for keyword in &task_keywords {
            if keyword.len() > 2 {
                ...
                if let Some(matching) = symbol_index.get(*keyword) {
                    let file_matches: Vec<_> = matching.iter()
...
fn build_compressed_context(
    ...
    for file in ranked_files.iter().take(30) {
        let entry = format!("- {} (score: {:.1}, symbols: {})\n", 
            file.path.display(), 
            file.score,
            file.symbol_count
        );
    ...
    let important_symbols: Vec<_> = symbols.iter()
        .filter(|s| matches!(s.kind, SymbolKind::Function | SymbolKind::Struct | SymbolKind::Class | SymbolKind::Trait))
        .take(50)
        .collect();
