...
pub async fn generate_reproduction_test(
    ...
    let test_files = match req.repro_mode {
        ...
        ReproductionMode::IntegrationTest => generator.generate_integration_test(req).await?,
        ReproductionMode::PropertyBased => generator.generate_property_test(req).await?,
        ReproductionMode::EdgeCaseExplorer => generator.generate_edge_case_tests(req).await?,
    };

    let diagnostics = analyze_failure_signature(req, repo).await?;
    let suggested_fixes = infer_suggested_fixes(&diagnostics, repo);

    let reproduction_confidence =
        calculate_reproduction_confidence(&test_files, &diagnostics, &req.failure_description);
...
impl TestGenerator {
    ...
    async fn generate_minimal_test(&mut self, req: &ReproductionRequest) -> Result<Vec<PathBuf>> {
        ...
        if tests.is_empty() && !req.affected_files.is_empty() {
            let test = self
                .create_file_based_test(&req.affected_files[0], req)
                .await?;
    ...
    async fn generate_integration_test(
        ...
        if let Some(path) = self
            .write_test_file(&integration_test, "integration")
            .await?
        {
            tests.push(path);
        }
    ...
    fn find_symbol(&self, name: &str) -> Option<&CodeSymbol> {
        self.repo
            .symbols
            .iter()
            .find(|s| s.name == name || s.file.to_string_lossy().contains(name))
    }

    async fn create_unit_test_for_symbol(
        &self,
        symbol: &CodeSymbol,
        req: &ReproductionRequest,
    ) -> Result<TestContent> {
    ...
    async fn create_file_based_test(
        &self,
        file: &Path,
        req: &ReproductionRequest,
    ) -> Result<TestContent> {
...
"#,
    ...
    async fn create_edge_case_test(
        &self,
        edge_case: &EdgeCase,
        req: &ReproductionRequest,
        index: usize,
...
"#,
    ...
    async fn write_test_file(
        ...
        let test_filename = format!(
            "{}_{}_repro_{}.rs",
            content.test_name, suffix, self.test_counter
        );
...
"#,
        req.task, req.failure_description, symbol.name, symbol.name
    )
}
...
"#,
        req.task, req.failure_description, symbol.name, symbol.file, symbol.name
    )
}
...
"#,
        file, req.task, req.failure_description, file
    )
}
    ...
    for diag in diagnostics {
        let (fix_type, description) = match diag.category {
            DiagnosticCategory::ExceptionThrown => (
                FixType::ErrorHandling,
                "Add error handling for potential exceptions".into(),
            ),
            DiagnosticCategory::AssertionFailure => (
                FixType::LogicCorrection,
                "Review assertion logic and expected values".into(),
            ),
            DiagnosticCategory::Timeout => (
                FixType::ResourceCleanup,
                "Add timeout handling and resource cleanup".into(),
            ),
            _ => continue,
        };
