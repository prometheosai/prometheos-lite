...
impl TrustReportBuilder {
    ...
    pub fn build(result: &HarnessExecutionResult, evidence_log: &EvidenceLog) -> TrustReport {
        ...
        TrustReport {
            ...
            started_at: evidence_log.started_at.map(|d| d.to_rfc3339()).unwrap_or_default(),
    ...
    fn build_generation_section(
        ...
        TrustSection {
            ...
            summary: format!("{} patch candidate(s) were generated by the AI.", stats.patches_generated),
            details,
            evidence_ids,
        }
    }
    ...
    fn calculate_trust_score(sections: &[TrustSection], stats: &TrustStatistics) -> u32 {
        let mut score = 100;
    ...
    fn generate_recommendations(sections: &[TrustSection], stats: &TrustStatistics) -> Vec<String> {
        ...
        if stats.files_modified > 10 {
            recommendations.push(
                "Many files were modified - review carefully before committing.".to_string(),
            );
        }

        if recommendations.is_empty() && stats.critical_issues == 0 {
            recommendations.push(
                "The changes appear safe to review and accept.".to_string(),
            );
        }
