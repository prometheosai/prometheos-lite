...
impl TaskLocalKnowledgeCache {
    ...
    pub async fn set(&self, scope: CacheScope, key: impl Into<String>, value: CacheValue, ttl: Option<Duration>) -> Result<()> {
        let scope_str = scope.as_str().to_string();
    ...
    pub async fn set_json<T: Serialize>(&self, scope: CacheScope, key: impl Into<String>, value: &T, ttl: Option<Duration>) -> Result<()> {
        let json_value = serde_json::to_value(value)?;
        self.set(scope, key, CacheValue::Json(json_value), ttl).await
    }
    
    pub async fn get_json<T: for<'de> Deserialize<'de>>(&self, scope: CacheScope, key: &str) -> Result<Option<T>> {
...
mod tests {
    ...
    async fn test_basic_cache_operations() {
        ...
        cache.set(CacheScope::Task, "key1", CacheValue::String("value1".into()), None).await.unwrap();
    ...
    async fn test_json_cache() {
        ...
        cache.set_json(CacheScope::Task, "data", &data, None).await.unwrap();
