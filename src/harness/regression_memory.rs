...
impl RegressionMemory {
        ...
        if let Some(pattern_ids) = self.type_index.get(&failure_type) {
            for id in pattern_ids {
                if let Some(pattern) = self.patterns.get(id) {
                    let similarity = self.calculate_similarity(pattern, error_message, &context_hash, file_path);
                    
                    if similarity > 0.7 {
                        let mut recommended = pattern.successful_solutions.clone();
                        recommended.sort_by(|a, b| b.confidence.partial_cmp(&a.confidence).unwrap());
    ...
    pub fn learn_solution(
        ...
        if let Some(pattern) = self.patterns.get_mut(pattern_id) {
            if success {
                ...
                if let Some(existing) = pattern.successful_solutions.iter_mut()
    ...
    pub fn get_memory_stats(&self) -> MemoryStats {
        ...
        let total_solutions: usize = self.patterns.values()
            .map(|p| p.successful_solutions.len())
            .sum();
        let total_failures: u32 = self.patterns.values()
            .map(|p| p.frequency)
            .sum();
        
        let patterns_with_solutions = self.patterns.values()
    ...
    pub fn import_memory(&mut self, patterns: Vec<FailurePattern>) {
        for pattern in patterns {
            self.patterns.insert(pattern.pattern_signature.clone(), pattern);
        }
    }

    fn generate_signature(&self, error_message: &str, file_path: Option<&Path>, line: Option<u32>) -> String {
        let file_part = file_path.map(|p| p.to_string_lossy().to_string()).unwrap_or_default();
        let line_part = line.map(|l| l.to_string()).unwrap_or_default();
        format!("{}:{}:{}", self.hash_string(error_message), file_part, line_part)
    }
