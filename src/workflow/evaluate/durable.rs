...
//!
//! Every mutable durable document (registry, identity, checkpoint) is
//! published through [`atomic_write_json`]: serialize → write temp file →
//! fsync → atomically rename → fsync the containing directory. Every step
//! propagates its error; a failed publication is reported to the caller and
