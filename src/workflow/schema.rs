...
//!
//! Every on-disk durable document owned by the workflow layer (the evaluation
//! pipeline's proposal registry, execution identity, evidence bundle,
//! evaluation checkpoint, and journal event, plus the portable work state)
//! carries a validated [`SchemaVersion`]. The version is the single
//! authoritative representation used for read-path compatibility checks,
//! migration dispatch, and fail-closed rejection of unknown future versions.
//!
//! Existing pre-1.6 files are unversioned. They are treated as an explicit
...
///
/// Serializes as a `"major.minor.patch"` string so existing documents (which
/// already carry `"schema_version": "1.0.0"`) remain byte-compatible.
...
pub enum DocumentType {
    ...
    JournalEvent,
    /// Portable work state document (see [`crate::workflow::portable_state`]).
    PortableWorkState,
}
...
impl DocumentType {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            DocumentType::PortableWorkState => "PortableWorkState",
        }
    }
...
mod tests {
    ...
    #[test]
    fn portable_work_state_uses_shared_version_machinery() {
        let doc = DocumentType::PortableWorkState;
        assert_eq!(doc.as_str(), "PortableWorkState");
        assert_eq!(
            validate_version(doc, SchemaVersion::new(1, 0, 0)).unwrap(),
            VersionStatus::Current
        );
