...
pub trait PlanOperations: Repository {
    fn create_plan(&self, work_context_id: &str, plan: &ExecutionPlan) -> anyhow::Result<ExecutionPlan>;
    fn get_plan(&self, work_context_id: &str) -> anyhow::Result<Option<ExecutionPlan>>;
    fn update_plan(&self, work_context_id: &str, plan: &ExecutionPlan) -> anyhow::Result<ExecutionPlan>;
...
impl PlanOperations for crate::db::Db {
    ...
    fn get_plan(&self, work_context_id: &str) -> anyhow::Result<Option<ExecutionPlan>> {
        ...
        let mut rows = stmt.query_map(params![work_context_id], |row| {
    ...
    fn delete_plan(&self, work_context_id: &str) -> anyhow::Result<()> {
        ...
        conn.execute("DELETE FROM execution_plans WHERE work_context_id = ?1", params![work_context_id])
