...
impl FileParser {
            ...
            if let Some(header_caps) = header_regex.captures(&output[pos..]) {
                ...
                if let Some(code_caps) = code_block_regex.captures(&output[pos..]) {
                    ...
                    if let Some(path) = current_path.take() {
                        let language = code_caps.get(1).map(|m| m.as_str().to_string()).unwrap_or_else(|| "text".to_string());
                        let content = code_caps.get(2).map(|m| m.as_str().to_string()).unwrap_or_default();
    ...
    pub fn extract_file_blocks(output: &str) -> Result<Vec<(String, String, String)>> {
        ...
        Ok(files.into_iter().map(|f| (f.path, f.content, f.language)).collect())
    }
}
