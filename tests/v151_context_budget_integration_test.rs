...
use prometheos_lite::context::{ContextBuilder, ContextBudgeter, ContextInputs, Artifact};
...
async fn test_context_budget_enforcement_integration() {
    ...
    for i in 0..50 {
        memory_items.push(Memory {
            ...
            content: format!("Memory item {} with some content that should be dropped due to budget constraints", i),
            summary: None,
            embedding: None,
    ...
    let result = builder.build(context_inputs).expect("Context build should succeed");
    
    // Assertions
    assert!(result.token_count <= 100, "Token count {} should be <= budget 100", result.token_count);
    
    // Low-priority items should have been dropped
    assert!(!result.dropped_items.is_empty(), "Some items should have been dropped");
    ...
    assert_eq!(result.metadata.memory_count, 50, "Should have tracked 50 memory items");
}
...
async fn test_context_budget_with_artifacts() {
    ...
    assert!(result.token_count <= 50, "Token count should respect budget");
    assert!(!result.dropped_items.is_empty(), "Artifacts should be dropped");
    assert_eq!(result.metadata.artifact_count, 20, "Should track artifact count");
}
...
async fn test_context_budget_json_preservation() {
    ...
    if result.prompt.contains("{") {
        ...
        assert!(result.prompt.contains("}") || result.prompt.contains("Content too large"), 
