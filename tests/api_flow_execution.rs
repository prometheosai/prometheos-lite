...
use prometheos_lite::intent::{IntentClassifier, Intent, FlowSelector, DefaultFlowSelector};
...
fn test_build_flow_from_yaml() {
    ...
    for node_def in &flow_file.nodes {
        let node = factory.create(&node_def.node_type, node_def.config.clone()).unwrap();
...
async fn test_intent_classification_question() {
    let classifier = IntentClassifier::new().unwrap();
    let result = classifier.classify("What is the capital of France?").await.unwrap();
...
async fn test_intent_classification_coding() {
    let classifier = IntentClassifier::new().unwrap();
    let result = classifier.classify("Write a function to sort an array").await.unwrap();
...
async fn test_intent_classification_planning() {
    let classifier = IntentClassifier::new().unwrap();
    let result = classifier.classify("Plan a project to build a web app").await.unwrap();
...
fn test_shared_state_with_personality_mode() {
    ...
    assert_eq!(state.get_input("message"), Some(&serde_json::json!("test message")));
