...
fn test_codegen_flow_exists() {
    ...
    assert!(
        flow_path.exists(),
        "codegen.flow.json should exist in examples/"
    );
}
...
fn test_codegen_flow_valid_json() {
    ...
    let json: serde_json::Value =
        serde_json::from_str(&content).expect("Flow file should be valid JSON");

    // Verify required fields exist
    assert!(
        json.get("version").is_some(),
        "Flow should have version field"
    );
    assert!(json.get("name").is_some(), "Flow should have name field");
    assert!(json.get("nodes").is_some(), "Flow should have nodes field");
    assert!(
        json.get("transitions").is_some(),
        "Flow should have transitions field"
    );
    assert!(
        json.get("start_node").is_some(),
        "Flow should have start_node field"
    );
}
...
fn test_codegen_flow_structure() {
    ...
    assert!(
        node_ids.contains(&"planner"),
        "Flow should have a planner node"
    );
    assert!(node_ids.contains(&"coder"), "Flow should have a coder node");
    assert!(
        node_ids.contains(&"reviewer"),
        "Flow should have a reviewer node"
    );
    assert!(
        node_ids.contains(&"file_writer"),
        "Flow should have a file_writer node"
    );
    assert!(
        node_ids.contains(&"memory_write"),
        "Flow should have a memory_write node"
    );
}
...
fn test_codegen_flow_transitions() {
    ...
    let start_node = json["start_node"]
        .as_str()
        .expect("start_node should be a string");
    ...
    let mut transition_map: std::collections::HashMap<(String, String), String> =
        std::collections::HashMap::new();
    for t in transitions {
        if let (Some(from), Some(action), Some(to)) =
            (t["from"].as_str(), t["action"].as_str(), t["to"].as_str())
        {
