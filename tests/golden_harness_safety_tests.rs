...
use prometheos_lite::harness::{
    ...
    execution_loop::{HarnessExecutionRequest, ValidationFailurePolicy, execute_harness_task},
...
async fn test_no_provider_no_edits_blocks() -> Result<()> {
    ...
    assert!(
        error_msg.contains("No LLM provider configured")
...
async fn test_static_provider_unavailable_in_production() -> Result<()> {
    ...
    impl PatchProvider for StaticProvider {
        ...
        async fn generate(
            &self,
            _request: prometheos_lite::harness::patch_provider::GenerateRequest,
    ...
    assert!(
        error_msg.contains("validation")
            && (error_msg.contains("zero")
                || error_msg.contains("no commands")
...
async fn test_patch_hash_verification() -> Result<()> {
    use prometheos_lite::harness::patch_applier::{PatchHashVerification, compute_patch_hash};
...
async fn test_sandbox_evidence_recorded() -> Result<()> {
    ...
    let entry = evidence_log.record_sandbox_evidence(
        &docker_evidence,
        Some("cargo test"),
    ...
    assert_eq!(
        entry.kind,
        prometheos_lite::harness::evidence::EvidenceEntryKind::SandboxBackendUsed
    );
    ...
    assert!(
        entry
            .input_summary
            .values()
            .any(|v| v.contains("isolated_process") && v.contains("true"))
    );
    assert!(
        entry
            .input_summary
            .values()
            .any(|v| v.contains("network_disabled") && v.contains("true"))
    );
    ...
    let local_entry = evidence_log.record_sandbox_evidence(
        &local_evidence,
        Some("cargo build"),
        Some("trace-9-local".to_string()),
    );
    ...
    assert!(
        local_entry
            .input_summary
            .values()
            .any(|v| v.contains("Local"))
    );
    assert!(
        local_entry
            .input_summary
            .values()
            .any(|v| v.contains("isolated_process") && v.contains("false"))
    );
...
async fn test_validation_command_counters() -> Result<()> {
    use prometheos_lite::harness::{evidence::EvidenceLog, validation::ValidationCategory};
    ...
    assert_eq!(
        entry.input_summary.get("commands_planned"),
        Some(&"5".to_string())
    );
    assert_eq!(
        entry.input_summary.get("commands_executed"),
        Some(&"4".to_string())
    );
    assert_eq!(
        entry.input_summary.get("commands_skipped"),
        Some(&"1".to_string())
    );
    assert_eq!(
        entry.input_summary.get("categories_count"),
        Some(&"3".to_string())
    );
    ...
    assert_eq!(
        zero_entry.input_summary.get("commands_planned"),
        Some(&"0".to_string())
    );
    assert_eq!(
        zero_entry.input_summary.get("commands_executed"),
        Some(&"0".to_string())
    );
    ...
    pub fn verify_completion_safety(evidence: &CompletionEvidence) -> Result<()> {
        ...
        if evidence.process_evidence.all_phases_completed {
            let has_docker_evidence = evidence.sandbox_evidence.iter().any(|e| {
                matches!(
                    e.runtime_kind,
                    prometheos_lite::harness::sandbox::SandboxRuntimeKind::Docker
                ) && e.isolated_process
                    && e.isolated_filesystem
                    && e.network_disabled
            });
