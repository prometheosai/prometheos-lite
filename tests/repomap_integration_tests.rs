...
"#,
    ...
    assert!(
        !repo_map.modules.is_empty(),
        "Should have extracted modules"
    );

    let lib_module = repo_map
        .modules
        .iter()
        .find(|m| m.name == "lib")
    ...
    assert!(
        lib_module.functions.len() >= 2,
        "Should extract at least 2 functions"
    );

    let function_names: Vec<String> = lib_module
        .functions
        .iter()
        .map(|f| f.name.clone())
        .collect();
    ...
    assert!(
        function_names.contains(&"multiply".to_string()),
        "Should find multiply function"
    );
    assert!(
        function_names.contains(&"helper".to_string()),
...
"#,
    ...
    assert!(
        types_module.structs.len() >= 2,
        "Should extract at least 2 structs"
    );
    assert!(
        types_module.enums.len() >= 1,
        "Should extract at least 1 enum"
    );

    let struct_names: Vec<String> = types_module
        .structs
        .iter()
        .map(|s| s.name.clone())
        .collect();
    ...
    assert!(
        struct_names.contains(&"Config".to_string()),
        "Should find Config struct"
    );

    let status_enum = types_module
        .enums
        .iter()
    ...
    assert!(
        !status_enum.variants.is_empty(),
        "Should extract enum variants"
    );
...
"#,
    ...
    assert!(
        traits_module.traits.len() >= 1,
        "Should extract at least 1 trait"
    );

    let drawable_trait = traits_module
        .traits
        .iter()
    ...
    assert!(
        !drawable_trait.methods.is_empty(),
        "Should extract trait methods"
    );

    // Check impl blocks
    let impl_blocks = traits_module
        .impl_blocks
        .iter()
...
"#,
    ...
    let visibility_module = repo_map
        .modules
        .iter()
    ...
    let public_funcs = visibility_module
        .functions
        .iter()
        .filter(|f| f.is_public)
        .count();

    let private_funcs = visibility_module
        .functions
        .iter()
        .filter(|f| !f.is_public)
        .count();
    ...
    assert!(
        private_funcs >= 1,
        "Should have at least 1 private function"
    );

    // Check module visibility
    let public_mods = visibility_module
        .submodules
        .iter()
        .filter(|m| m.is_public)
        .count();

    let private_mods = visibility_module
        .submodules
        .iter()
        .filter(|m| !m.is_public)
        .count();
...
"#,
    ...
    let documented_module = repo_map
        .modules
        .iter()
    ...
    assert!(
        documented_module.documentation.is_some(),
        "Should extract module documentation"
    );
    let module_doc = documented_module.documentation.as_ref().unwrap();
    assert!(
        module_doc.contains("utility functions"),
        "Should contain module description"
    );

    // Check function documentation
    let add_function = documented_module
        .functions
        .iter()
        .find(|f| f.name == "add")
    ...
    assert!(
        user_struct.documentation.is_some(),
        "Should extract struct documentation"
    );
    let struct_doc = user_struct.documentation.as_ref().unwrap();
    assert!(
        struct_doc.contains("user in the system"),
        "Should contain struct description"
    );
...
"#,
    ...
    let inline_modules = inline_module
        .submodules
        .iter()
    ...
    let utils_module = inline_module
        .submodules
        .iter()
        .find(|m| m.name == "utils")
    ...
    assert!(
        utils_module.functions.len() >= 1,
        "Should extract functions from inline module"
    );

    // Check nested inline module
    let internal_modules = utils_module
        .submodules
        .iter()
    ...
    assert!(
        !internal_modules.is_empty(),
        "Should find nested inline module"
    );
...
"#,
    ...
    let repository_struct = complex_module
        .structs
        .iter()
    ...
    assert!(
        repository_struct.generics.is_some(),
        "Should extract generic parameters"
    );

    // Check impl blocks with generics
    let impl_blocks = complex_module
        .impl_blocks
        .iter()
    ...
    assert!(
        config_impl.methods.len() >= 2,
        "Should extract macro-generated methods"
    );

    // Check type aliases
    let type_aliases = complex_module
        .type_aliases
        .iter()
...
"#,
    ...
    assert!(
        !repo_map.modules.is_empty(),
        "Should still create module entry"
    );
