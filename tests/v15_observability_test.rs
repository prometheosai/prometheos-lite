...
use chrono::Utc;
use prometheos_lite::flow::tracing::{HierarchicalTrace, LlmCall, NodeRun, ToolCall, TraceStorage};
use uuid::Uuid;
...
fn test_trace_hierarchy() {
    ...
    let retrieved = storage
        .get_trace(&trace.trace_id.to_string())
        .unwrap()
        .unwrap();
...
fn test_get_traces_by_flow_run() {
    ...
    trace1.flow_run_id = flow_run_id.clone();

    let mut trace2 = create_test_trace();
    trace2.flow_run_id = flow_run_id.clone();
...
fn test_trace_generation_correctness() {
    ...
    assert!(!retrieved.trace_id.is_empty());
...
fn test_failure_trace_visibility() {
    ...
    // Add a failed node with the same trace_id as the main trace
