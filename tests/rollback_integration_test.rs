...
use prometheos_lite::harness::{
    ...
    patch_applier::{apply_patch_with_rollback, dry_run_patch, RollbackHandle},
};
...
async fn test_rollback_file_edit() {
    ...
    assert!(!modified_content.contains("Hello"), "Old content still present");
    
    // Perform rollback
    let rollback_result = rollback_handle.rollback().await.expect("Rollback failed");
    
    // Verify rollback result
    assert_eq!(rollback_result.restored.len(), 1, "Should have 1 restored file");
    assert_eq!(rollback_result.deleted.len(), 0, "Should have 0 deleted files");
    assert_eq!(rollback_result.recreated.len(), 0, "Should have 0 recreated files");
    
    // Verify original content is restored
    let restored_content = fs::read_to_string(&file_path).await.unwrap();
    assert_eq!(restored_content, initial_content, "Content not restored correctly");
...
async fn test_rollback_create_file() {
    ...
    // Build file set and policy
    let file_set = FileSet::from_paths(vec![new_file_path.clone()]).await.unwrap();
    let policy = FilePolicy::default();
    ...
    assert!(created_content.contains("new_function"), "Wrong content in new file");
    
    // Perform rollback
    let rollback_result = rollback_handle.rollback().await.expect("Rollback failed");
    
    // Verify rollback result
    assert_eq!(rollback_result.restored.len(), 0, "Should have 0 restored files");
    assert_eq!(rollback_result.deleted.len(), 1, "Should have 1 deleted file");
    assert_eq!(rollback_result.recreated.len(), 0, "Should have 0 recreated files");
    
    // Verify the file was deleted
    assert!(!new_file_path.exists(), "New file was not deleted during rollback");
...
async fn test_rollback_delete_file() {
    ...
    assert_eq!(rollback_result.restored.len(), 0, "Should have 0 restored files");
    assert_eq!(rollback_result.deleted.len(), 0, "Should have 0 deleted files");
    assert_eq!(rollback_result.recreated.len(), 1, "Should have 1 recreated file");
    
    // Verify the file was recreated
    assert!(file_path.exists(), "File was not recreated during rollback");
    let restored_content = fs::read_to_string(&file_path).await.unwrap();
    assert_eq!(restored_content, file_content, "Wrong content in recreated file");
...
async fn test_rollback_conflict_detection() {
    ...
    fs::write(&file_path, external_content).await.expect("Failed to write external content");
    ...
    println!("✅ Conflict detection test passed (result: {:?})", rollback_result.is_ok());
}
...
"#;
    ...
    assert_eq!(after_rollback, initial_content, "Content not restored after rollback");
