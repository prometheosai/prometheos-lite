...
use prometheos_lite::harness::{
    ...
    work_integration::{HarnessWorkContextService, extract_task_hints},
};
...
"#,
    ...
    assert!(
        !result.evidence_log.entries.is_empty(),
        "Should have evidence log entries"
    );
...
"#,
    ...
    for trace_id in &trace_ids_2 {
        assert_eq!(
            trace_id, &trace_id_2,
            "Trace ID should be consistent in blocked path"
        );
    }
...
"#,
    ...
    let completion_entries: Vec<_> = result
        .evidence_log
        .entries
        .iter()
...
async fn test_p0_5_runtime_factory_usage() -> Result<()> {
    ...
    assert!(
        local_runtime
            .as_ref()
            .downcast_ref::<prometheos_lite::harness::sandbox::LocalCommandRuntime>()
            .is_some()
    );
...
"#,
    ...
    let validation_plan =
        prometheos_lite::harness::validation::ValidationPlan::default_for_repo(&env);
    ...
    match env.languages.first().map(|s| s.as_str()) {
        Some("rust") => {
            ...
            assert!(
                !validation_plan.lint_commands.is_empty(),
                "Rust repo should have lint commands"
            );
            assert!(
                !validation_plan.test_commands.is_empty(),
                "Rust repo should have test commands"
            );
        }
        _ => {
...
"#,
    ...
    assert!(
        fresh_plan.disable_cache,
        "Fresh validation plan should have cache disabled"
    );
...
"#,
    ...
    let evidence_log_artifacts: Vec<_> = updated_ctx
        .artifacts
        .iter()
    ...
    assert!(
        !evidence_log_artifacts.is_empty(),
        "Should have EvidenceLog artifact"
    );
...
"#,
    ...
    assert!(matches!(
        result_no_provider.completion_decision,
        CompletionDecision::Blocked(_)
    ));
...
"#,
    ...
    let req = HarnessExecutionRequest {
        ...
        repo_root: repo_root.to_path_buf(),
        task: "Add email field to User struct in src/lib.rs and update create_user function"
            .to_string(),
        requirements: vec![
    ...
    let evidence_trace_ids: Vec<_> = result
        .evidence_log
        .entries
        .iter()
        .filter_map(|e| e.trace_id.as_ref())
    ...
    for evidence_trace_id in evidence_trace_ids {
        assert_eq!(
            ...
            "All evidence should have same trace ID"
        );
    }
